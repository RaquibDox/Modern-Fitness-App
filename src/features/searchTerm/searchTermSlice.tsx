...
export const fetchSearchTerms = createAsyncThunk(
  ...
  async (): Promise<string[]> => {
    try {
      const targetList = await axios.request({
        ...exerciseOptions,
        url: BASE_URL[0],
      });
      const equipmentList = await axios.request({
        ...exerciseOptions,
        url: BASE_URL[1],
      });
...
const searchTermSlice = createSlice({
  ...
  extraReducers(builder) {
    builder.addCase(fetchSearchTerms.fulfilled, (state, action) => {
...
export const getSearchTerms = (state: SearchStateType) =>
